const setHighlightWords = (messageElement) => {
  var newc = [];
  var messageElement = document.getElementsByClassName(
  ...
  chrome.storage.local.get(
    ...
    (r) => {
      if (r.highlightEnable !== undefined && r.highlightEnable !== 'off') {
        if (r.highlightKeywords && r.highlightColor) {
          newc = r.highlightKeywords.split(',');
            ...
            for (var l = 0; l < textassplit.length; l++) {
              for (var j = 0; j < newc.length; j++) {
                if (textassplit[l].includes(newc[j])) {
                  ...
                  messageElement[x].style.opacity = r.highlightOpacity;
                  messageElement[x].style.backgroundColor = r.highlightColor;
                }
              }
            }
          }
